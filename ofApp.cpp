...
void ofApp::setup() {
	...
	auto file_path = "D:\\MP4\\VN20210917_145135.mp4";
...
void ofApp::update() {
	...
	for (int i = this->box2d_rect_list.size() - 1; i > -1; i--) {
		...
		if (this->box2d_life_list[i] < 0) {
			...
			this->box2d_color_list.erase(this->box2d_color_list.begin() + i);
		}
	}
		...
		if (frame_index < next_frame_index) {
			...
			for (int i = 0; i < this->cv_rect_list.size(); i++) {
				...
				if ((gv[0] + gv[1] + gv[2]) > 80) {
					...
					this->box2d_color_list.push_back(base_color_list[ofRandom(base_color_list.size())]);
				}
			}
		}
	}
...
void ofApp::draw() {
	...
	for (int i = 0; i < this->box2d_rect_list.size(); i++) {
		...
		ofSetColor(this->box2d_color_list[i], this->box2d_life_list[i] > 128 ? 192 : ofMap(this->box2d_life_list[i], 128, 0, 192, 0));
		ofFill();
		ofDrawPlane(glm::vec3(), this->rect_size, this->rect_size);
	...
	ofSetLineWidth(3);
	ofSetColor(39);
	ofDrawRectangle(glm::vec3(), 720, 720);
